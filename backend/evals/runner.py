"""Run the Agent 1 eval set against the real model and score the results.
    ...
    python -m backend.evals.runner                 # run every case once (30 calls)
    python -m backend.evals.runner --tag hard      # only the hard set (12 calls)
    python -m backend.evals.runner --repeat 3      # measure consistency across runs
    python -m backend.evals.runner --tag regression  # only the must-never-break cases
    python -m backend.evals.runner --delay 1.0     # slow down if rate limited
...
from backend.agents.profile_agent import create_investor_profile
from backend.config import PROJECT_ROOT, get_settings
from backend.evals.cases import CASES, EvalCase, check_expectations
