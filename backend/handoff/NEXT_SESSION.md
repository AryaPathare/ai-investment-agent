...
**A2. `python -m backend.evals.company_runner` against the buyer ceiling (entry 106).**
...
Expect **1062 passed, 1 skipped** - 1063 collected, and the distinction matters
...
session 19, 1040 after session 21, and 1062 after session 22, which added the
repository-hygiene guard and then the website's page-structure guards.
...
Ran in 255s, exit 0, run id **`cli-163fffe8`**. `python -m backend.cli --resume
...
`python -m backend.scripts.build_log_html` renders `docs/PROJECT_LOG.md` to
...
The widest brief this pipeline has produced, and the one `python -m backend.cli --demo`
...
python -m backend.evals.research_runner         # Agent 2: process quality, 5 profiles
python -m backend.evals.company_runner          # Agent 3: runs 2 -> 3
python -m backend.evals.risk_runner             # Agent 4: runs 2 -> 3 -> 4
python -m backend.evals.decision_runner         # Agent 5: runs 2 -> 3 -> 4 -> 5
python -m backend.evals.decision_runner --case <name>   # one profile, to conserve quota
```
...
- **Groq's daily ceiling is the binding constraint.** A profile through Agents
  ...
  at about 12 model calls per profile. A full `python -m backend.cli` run costs the same
