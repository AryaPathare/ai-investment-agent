...
def run(graph, thread_id: str, start, opening: tuple[int, str]) -> dict:
    ...
    * ``None``                  picking up a run killed partway through a stage
    ...
    print(f"  python -m backend.cli --resume {thread_id}")
...
def print_saved_runs(runs: list) -> int:
    ...
    if resumable:
        print()
        print(_wrap(
            f"Resume one with: python -m backend.cli --resume {resumable[0].thread_id}",
            indent="  ",
        ))
    return 0
...
def resume(store, thread_id: str) -> int:
    ...
    if saved is None:
        ...
        print("Run python -m backend.cli --list to see what is saved.")
        return 1
    ...
    try:
        ...
        with checkpoints.open_store(args.db) as store:
            ...
            if store.run(thread_id) is not None:
                ...
                print("  Continue it:  python -m backend.cli --resume " + thread_id)
        ...
        print()
        print()
        print("Stopped. The run is saved; python -m backend.cli --list will show it.")
        return 1
