...
## Session 26 — 2026-09-09

### 136. The page is dark, and the type finally does more than one thing

He called the blue-and-black page "monotone" and "boring" and asked for modern,
futuristic, enthusiastic. Five reference sites, given with what he liked about
each:

    obsidianintelligence.ai   favourite overall - design and animation
    dotlogics.com             the animation
    cielhomes.in              the theme, and the fonts, which "make it look fun"
    racescout.ai              the layout
    561roofers.com            the layout

**They were screenshotted and probed rather than described.** A Playwright pass
took three captures of each and read back the computed fonts, weights, palette,
radii and keyframe counts. The point was to find what they share, and what they
share is not a colour:

    obsidian    #08080a  IBM Plex Sans + Mono, Archivo 100px/200, mint accent
    racescout   near-black hero, Space Grotesk 78px/700 -2.34px, lime accent
    ciel        #142a1d  Cormorant Garamond 100px/300, gold accent, DM Sans
    roofers     Manrope 93.6px/900 -3.74px
    dotlogics   Aeonik, 38 keyframe blocks, 636 animated rules

**CONTRAST OF VOICE.** A near-black ground, one electric accent used sparingly,
a display face set large with tight tracking, and small mono labels in wide
caps. That is the diagnosis of "monotone" the complaint could not give: the old
page had ONE font and ONE hue, so every line looked like every other line.

### What the redesign actually is

A new stylesheet and a motion layer. **The markup and every word are the
previous page's**, and that was enforced rather than intended: the design was
generated by a script that read `index.html`, swapped the `<style>` block, added
a font link, wrapped one word of the title in a span, and appended a script -
then compared the visible text of both pages and refused to write if a single
word had changed.

Four voices where there was one:

    Cormorant Garamond  headings only. 300 weight, italic on the title's last
                        word and on every h2.
    Inter               anything read as a sentence.
    JetBrains Mono      anything read as a token: labels, counts, ids, 01-05.
    Georgia             the brief, unchanged - it is a document, not interface.

The title is lifted from Ciel exactly, measured off their page: Cormorant 300,
letter-spacing -.01em, upright words then an italic last word that changes
colour. Theirs is gold on green; this one is cyan on navy.

**A script face was the other reading of "curly" and is the wrong tool here.**
At 14px a formal script stops being legible, and these headings carry tickers
and currency amounts down to 360px.

### Three things that only showed up once it was built

**Cormorant draws its numeral 1 with no foot serif**, so the paper card read
"I5 pages" - a number rendering as a letter, on a card whose only job is to
state a number. Every figure moved to the mono. The rule that came out of it is
worth keeping: **words are set in the fancy face, tokens are not.**

**The dropdowns flashed white on opening.** Three causes were found and fixed -
the page declared no `color-scheme`, so Chrome assumed light and painted the
list white before the `option` rule repainted it; the controls were
translucent, which an OS-drawn popup cannot be; and they ANIMATED their
background on focus, so the popup took its colour from a control 300ms into a
transition. **The flash survived all three.** What remains is Chromium creating
the popup as a separate native window with a default white surface before its
first paint - outside the page, and unreachable by CSS. The only cure is
replacing `<select>` with a custom listbox, which costs the keyboard and
screen-reader behaviour that comes free, and costs the native picker on a
phone, which is better than anything hand-rolled. **Left alone on his call.**

**The lede and the stat tiles sat outside the tabs**, so they were on all three
- and on About they restated, nearly word for word, the paragraph directly
beneath them. Both moved into the run tab. The DISCLAIMER did not: a test pins
it outside the tab panels, and its docstring says why - "an About tab is
exactly the kind of change that quietly relocates a disclaimer into a page
nobody opens". It became a standing amber notice above the tabs instead, which
is more visible than where it was.

### The motion, and the one rule it follows

A scroll-progress hairline, sections that rise once as they arrive, two aurora
blooms drifting behind a technical grid, the 01-05 rails filling in sequence, a
bloom that lags the cursor, a highlight that tracks the pointer inside each
card, and a pulse on the stage a run is currently working on.

**All of it is decoration except that last one**, and all of it is off under
`prefers-reduced-motion` - verified by loading the page with the flag set and
counting elements left at opacity 0, which is zero. The pointer effects are
additionally gated on `(hover: hover) and (pointer: fine)`: on a touch profile
the cursor light **is never created**, because a light with no pointer either
never appears or freezes where the last tap landed.

### A test that broke for the wrong reason

`test_the_headline_scales_down_on_a_narrow_screen` asserted the exact string
`clamp(2rem, 8vw, 2.9rem)`. The retuned headline broke it while the page was
still correct on a phone - a guard failing for the wrong reason, which teaches
people to edit the test rather than read it. It now asserts the PROPERTY: a
clamp, a viewport-relative middle term, and a floor that fits 320px. Verified
against both designs and against a mutation that replaces the clamp with a
fixed size. Same lesson as entry 135, two entries later.
